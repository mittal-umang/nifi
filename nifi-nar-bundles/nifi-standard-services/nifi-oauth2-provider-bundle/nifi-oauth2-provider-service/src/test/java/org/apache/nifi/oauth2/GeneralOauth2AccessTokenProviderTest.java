...
class GeneralOauth2AccessTokenProviderTest {
    ...
    public void setUp() {
        ...
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.GRANT_TYPE).getValue()).thenReturn(
                StandardOauth2AccessTokenProvider.RESOURCE_OWNER_PASSWORD_CREDENTIALS_GRANT_TYPE.getValue());
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.AUTHORIZATION_SERVER_URL)
                .evaluateAttributeExpressions().getValue()).thenReturn(AUTHORIZATION_SERVER_URL);
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.USERNAME).evaluateAttributeExpressions()
                .getValue()).thenReturn(USERNAME);
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.PASSWORD).getValue()).thenReturn(PASSWORD);
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.CLIENT_ID).evaluateAttributeExpressions()
                .getValue()).thenReturn(CLIENT_ID);
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.CLIENT_SECRET).getValue()).thenReturn(
                CLIENT_SECRET);
        when(mockContext.getProperty(StandardOauth2AccessTokenProvider.REFRESH_WINDOW)
                .asTimePeriod(eq(TimeUnit.SECONDS))).thenReturn(FIVE_MINUTES);
    ...
    public void testInvalidWhenClientCredentialsGrantTypeSetWithoutClientId() throws Exception {
        ...
        runner.setProperty(testSubject, StandardOauth2AccessTokenProvider.AUTHORIZATION_SERVER_URL,
                "http://unimportant");

        // WHEN
        runner.setProperty(testSubject, StandardOauth2AccessTokenProvider.GRANT_TYPE,
                StandardOauth2AccessTokenProvider.CLIENT_CREDENTIALS_GRANT_TYPE);
    ...
    public void testIOExceptionDuringRefreshAndSubsequentAcquire() throws Exception {
        ...
        checkLoggedRefreshError(
                new UncheckedIOException("OAuth2 access token request failed", new IOException(refreshErrorMessage)));

        checkError(new UncheckedIOException("OAuth2 access token request failed", new IOException(acquireErrorMessage)),
                actualException);
    }
    ...
    public void testHTTPErrorDuringRefreshAndSubsequentAcquire() throws Exception {
        ...
        List<String> expectedLoggedError = Arrays.asList(
                String.format("OAuth2 access token request failed [HTTP %d], response:%n%s", 500,
                        errorRefreshResponseBody),
                String.format("OAuth2 access token request failed [HTTP %d], response:%n%s", 503,
    ...
    public void testHTTPErrorDuringRefreshSuccessfulSubsequentAcquire() throws Exception {
        ...
        List<String> expectedLoggedError = Collections.singletonList(
                String.format("OAuth2 access token request failed [HTTP %d], response:%n%s", 500,
                        expectedRefreshErrorResponse));
